...
AIChat 是一款桌面 AI 聊天工具，面向希望统一管理多个大模型平台的个人用户与开发者。软件提供本地化的会话管理能力，支持将不同平台、不同模型集中到一个界面中完成配置、切换和对话。
...
- 流式输出：支持  流式返回，回复内容可以边生成边显示。
- 会话管理：支持新建、删除、重命名、置顶、拖拽排序和历史持久化。
- **多版本回答与导航**：支持多次刷新（重新生成）同一问题的回答，以历史回答快照（Alternatives）形式在数据库中完整记录每个版本的文本内容、所用模型、平台、各项 Token 消费及请求耗时，支持在 UI 上一键切换并同步刷新显示所有元数据。
- **顺滑跟随滚动与焦点锁定**：优化了长文本流式输出时的“跟随滚动”体验，智能追踪组件底边；切换不同长度版本回答时自动锁定翻页导航区域，避免页面无序跳跃，保持阅读焦点稳定。
- **高精度耗时监控**：实时测量 API 从发出请求到最后一段流式响应到达的高精度耗时，并完美记忆在各回答版本快照中。
